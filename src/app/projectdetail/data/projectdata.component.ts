...
export class ProjectdataComponent implements OnInit {
		...
		this.dataSubscript = this._route.parent.data.subscribe((data: { fullProject: IFullproject }) => {
			...
			for (var pdu = 0; pdu < this.projectData.length; pdu++) {
				...
				if (this.projectData[ind].portal_url !== undefined && !this.projectData[ind].portal_url.startsWith('http')) {
					...
					this.putDHsubscript = this._projDetService.putDatahost(this.projectData[ind].data_host_id, this.projectData[ind]).subscribe((r: IDatahost) => {
						r.isEditing = false;
			...
			if (this.neededUpdating) this._projDetService.setProjectData(this.projectData);

			this.messageSubscript = this.messageSubscript = this._dialogService.MessageToShow.subscribe((m: string) => {
				this.messageToShow = m;
			});
		});
		this.pdataSubscript = this._projDetService.projData().subscribe((d: Array<IDatahost>) => {
			this.projectData = d;
		});
	...
	public saveDatahost(d: IDatahost, i: number) {
		if ((d.description == undefined || d.description == "") &&
			...
			delete d.isEditing;
			this.putDHsubscript = this._projDetService.putDatahost(d.data_host_id, d).subscribe((r: IDatahost) => {
				//  alert("data host updated");
				r.isEditing = false;
				this.projectData[i] = r;
				this._projDetService.setProjectData(this.projectData);
	...
	public AreYouSureDialogResponse(val: boolean) {
		...
		if (val) {
			...
			if (this.messageToShow == "Are you sure you want to change tabs? Any unsaved information will be lost.") {
				...
				//delete it
				this.deleteDHsubscript = this._projDetService.deleteDatahost(this.deleteID).subscribe(
					result => {
